...
class AttendanceHelper:
    ...
    def captureVideo(self, known_face_encodings, person_names, camera_id, scale_factor, box_color, text_color, font_scale, line_thickness, window_name, exit_key):
        ...
        video = cv2.VideoWriter("{0}{1}.mp4".format('attendance_videos/', date), cv2.VideoWriter_fourcc(*'mp4v'), 10, frame_size) # write video file
        while True:
        ...
        #rename video
        old_video = os.path.join("attendance_videos", "{0}.mp4".format(date))
        new_video = os.path.join("attendance_videos", "{0}_{1}.mp4.".format(selected_person, date))
